...
Este apêndice reúne os diagramas de sequência detalhados (\gls{SD}) que expandem os \glspl{SSD} apresentados no Capítulo~\ref{cap:analise_desenho}. A sua colocação em anexo permite documentar os principais fluxos sem interromper a leitura das evidências visuais do Capítulo~\ref{cap:implementação}. Os modelos de domínio e as demonstrações da interface mantêm-se no texto principal, uma vez que são necessários para compreender diretamente a solução desenvolvida. As Figuras~\ref{fig:sd_login_full} a~\ref{fig:sd_kyb_verification_full} apresentam os fluxos detalhados correspondentes.

%----------------------------------------------------------------------------------------

\section{Autenticação e resolução de acesso}
\label{app:sd_login_full}

A Figura~\ref{fig:sd_login_full} detalha a autenticação do operador e a resolução inicial do seu contexto de acesso.
...
\caption{SD detalhado: autenticação e resolução inicial do operador}
