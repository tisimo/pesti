...
No caso deste projeto, o \textit{backoffice} foi também concebido como uma base multiaplicação. No estado atual, existem o módulo transversal de administração dos próprios operadores, perfis e permissões do \textit{backoffice}, e a aplicação JustCauses como primeiro domínio funcional. Ainda assim, a estrutura foi desenhada para facilitar a integração futura de outras aplicações no mesmo \textit{backoffice}, integrando uma autenticação comum e reutilizando controlo de acesso, auditoria e navegação base.
...
Controlo de acesso & O \textit{backoffice} deve distinguir operadores, perfis, aplicações, páginas e ações, aplicando permissões no \textit{front-end} e no \textit{back-end}. \\ \hline
...
Fluxos operacionais & O sistema deve suportar processos internos, como revisão de campanhas, gestão de categorias, verificações \gls{KYC}/\gls{KYB} e administração de acessos. \\ \hline
Expansão do \textit{backoffice} & O \textit{backoffice} deve permitir a integração futura de novas aplicações, integrando uma autenticação comum e reutilizando operadores, perfis, permissões, auditoria e navegação base. \\ \hline
...
A análise de trabalhos relacionados parte de exemplos públicos que permitem observar partes do funcionamento de \textit{backoffices} reais e de ferramentas configuráveis para aplicações internas. Nos painéis comerciais é possível retirar referências sobre organização operacional, permissões e rastreabilidade. Nas ferramentas configuráveis, a questão é diferente: importa perceber até que ponto uma solução pronta substituiria desenvolvimento próprio. A limitação principal mantém-se, porque muitos fluxos de \textit{backoffice} só estão disponíveis a equipas internas, clientes autenticados ou planos comerciais específicos.
...
\begin{itemize}
  \item Domínio fortemente orientado a comércio eletrónico, sem correspondência direta com todos os fluxos do \textit{backoffice} desenvolvido.
...
A comparação mostra uma limitação comum: nenhum exemplo cobre, sozinho, o conjunto de requisitos deste projeto. Os produtos comerciais são úteis como referência de organização e controlo de acesso, mas não podem ser reutilizados. As ferramentas configuráveis reduzem o esforço em aplicações internas simples; perdem força quando é necessário combinar autorização por aplicação, auditoria própria, fluxos de decisão e evolução para novas aplicações no mesmo \textit{backoffice}. Por esse motivo, a solução à medida ficou mais ajustada ao contexto da empresa.
...
Mais do que comparar tecnologias de forma genérica, o estado da arte deve identificar os princípios que condicionam o desenho de um painel de administração. No projeto desenvolvido, destacam-se três: autorização aplicada de forma consistente, auditoria operacional e separação entre ferramentas genéricas e implementação à medida. A autenticação é tratada como uma dependência comum da entrada no sistema; a análise deste relatório concentra-se sobretudo no que acontece depois dessa identificação, isto é, no controlo de acesso, permissões, auditoria e evolução multiaplicação. Estes princípios são especialmente relevantes num painel multiaplicação, onde permissões e registos de atividade devem manter significado mesmo quando novos domínios funcionais são acrescentados.
...
A questão relevante foi perceber se a base existente servia o \textit{backoffice} pretendido. No \textit{front-end}, React com TypeScript e Vite já permitiam criar componentes reutilizáveis, rotas protegidas e integração com serviços de autenticação. Como as páginas não tinham elevada complexidade visual, uma comparação profunda entre \textit{frameworks} de interface teria pouco impacto prático no trabalho. O problema mais importante estava na coerência entre navegação, permissões, seleção de aplicação e chamadas à \gls{API}.
...
A opção final foi manter a base técnica da empresa e construir o \textit{backoffice} à medida. Esta escolha teve razões práticas.

A arquitetura existente já tinha rotas, serviços, componentes e contratos de \gls{API} próprios. Introduzir uma ferramenta externa de administração obrigaria a encaixar esses elementos numa abstração diferente, com ganhos pouco claros para um projeto que não começou do zero.

A autorização também pesou na decisão. O sistema precisava de validar aplicação, página e ação no servidor, e não apenas esconder opções na interface. Para isso, o \textit{front-end}, o \textit{back-end} e a configuração administrativa tinham de partilhar o mesmo modelo de permissões.

A auditoria exigia a mesma integração. Registar ações internas e acessos negados não era um detalhe acrescentado no fim; fazia parte do desenho da solução e tinha de ficar ligado aos serviços e entidades do \textit{backoffice}.

Também pesou o crescimento previsto do ecossistema OnlyHighIQ. Um \textit{backoffice} à medida permite integrar uma autenticação comum e separar módulos transversais, como operadores, perfis, permissões e auditoria, de módulos específicos da JustCauses, como campanhas, categorias, verificações KYC/KYB e indicadores operacionais. Assim, a primeira aplicação suportada não condiciona a evolução futura do \textit{backoffice}.
...
A análise do estado da arte foi feita a partir de capacidades observáveis, não de equivalência direta entre produtos. O que mais interessava ao projeto era perceber como outros sistemas tratam controlo de acesso, auditoria, fluxos operacionais, integração com dados, manutenção e evolução para vários domínios. Como os \textit{backoffices} reais raramente expõem todos os seus fluxos, foi necessário cruzar documentação oficial, imagens públicas e ferramentas comparáveis.

Shopify Admin e Stripe Dashboard ajudaram a observar práticas de gestão de operadores, permissões e histórico de atividade, mas pertencem a produtos fechados. Directus e Appsmith mostram caminhos mais rápidos para criar ferramentas internas, embora se tornem menos adequados quando o domínio exige regras próprias de autorização, auditoria e evolução arquitetural. No contexto da OnlyHighIQ, fez mais sentido manter a base React, Node.js, PostgreSQL e AWS existente e construir uma solução própria, tratando a JustCauses como primeira aplicação integrada e não como limite definitivo do \textit{backoffice}.
